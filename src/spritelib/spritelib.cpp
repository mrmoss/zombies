...
/* Draw Animation!!! */
void spritelib::draw_anim(spritelib_tex tex,float img,
	int imgNum,float x,float y, float w,float h,
	float angle,bool glow) const
{
	if (glow) glBlendFunc(GL_ONE,GL_ONE); /* just add everything together */
	else glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA); /* erase previous stuff */
	...
	glBegin(GL_TRIANGLE_FAN);
	w*=0.5f, h*=0.5f; // half width and height
	glTexCoord2f(((1.0/static_cast<float>(imgNum))*img),1.0);
	glVertex2fv(rotate_2D(angle,x,y,-w,-h));
	glTexCoord2f(((1.0/static_cast<float>(imgNum))*img)+(1.0/static_cast<float>(imgNum)),1.0);
	glVertex2fv(rotate_2D(angle,x,y,+w,-h));
	glTexCoord2f(((1.0/static_cast<float>(imgNum))*img)+(1.0/static_cast<float>(imgNum)),0.0f);
	glVertex2fv(rotate_2D(angle,x,y,+w,+h));
	glTexCoord2f(((1.0/static_cast<float>(imgNum))*img),0.0f);
	glVertex2fv(rotate_2D(angle,x,y,-w,+h));
	glEnd();
}
...
spritelib_tex spritelib::read_tex(const char *filename) const
{
    ...
    {
        //Create Log File
        ofstream ostr("ERROR!!!DELETEMEAFTERLOOKING!!!.txt");
            ostr<<"Could not load texture \""<<filename<<"\".\n";
        ostr.close();

        //Throw Error
        throw runtime_error("Could not load texture \""+std::string(filename)+"\".");
    }
}
...
void spritelib_display(void)
{
	...
	glScalef(lib.screenscale/lib.screenw,-lib.screenscale/lib.screenh,0.0f);
	...
	{
	    lib.key_typed[key]=false;
	}

	for(int key=0;key<256;key++)
	{
	    lib.kb_pressed[key]=false;
	}
...
void spritelib_special(int key,int /*x*/,int /*y*/)
{
    spritelib &lib=spritelib_get();
    if(!lib.kb_check[key])
    {
        lib.kb_pressed[key]=true;
    }
    lib.kb_check[key]=true;
    glutPostRedisplay();
}

void spritelib_specialUp(int key,int /*x*/,int /*y*/)
{
	spritelib &lib=spritelib_get();
	lib.kb_check[key]=false;
	glutPostRedisplay();
}
...
void spritelib_run(const char *window_title,int screenw,int screenh)
{
	...
	glutSpecialFunc(spritelib_special);
	glutSpecialUpFunc(spritelib_specialUp);
...
bool keyboard_check(int key)
{
    ...
    if(key>0x1000)
    {
        return lib.key[key-0x1000]+lib.key[key-0x1020];
    }
    else
    {
        return lib.kb_check[key];
    }
...
bool keyboard_check_pressed(int key)
{
    ...
    if(key>0x1000)
    {
        return lib.key_typed[key-0x1000]+lib.key_typed[key-0x1020];
    }
    else
    {
        return lib.kb_pressed[key];
    }
